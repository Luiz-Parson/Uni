...
{
    class AccessControlDay
    {
        protected static readonly Logger Logger = new Logger();

        public static DataTable GenerateProductReport(string description, string epc, string sku, DateTime initialDate, DateTime endDate)
        {
        ...
        public static DataTable GetAllEntrysAndExits(string description, string epc, string sku, DateTime initialDate, DateTime endDate)
        {
            if (!string.IsNullOrEmpty(epc))
            {
                epc = epc.PadLeft(24, '0');
        ...
        public static long GetSequence()
        {
            ...
            {
                Logger.Error("Erro ao obter valor da sequência", ex);
                return -1; // Retorna -1 para indicar erro (pode ser tratado melhor conforme a necessidade)
            }
        }
    }
}
